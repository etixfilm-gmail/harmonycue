// hc-static-js-multimixer.js gets loaded by hc-views/layout/index.ejs
// as a <script> element at the bottom of the <body>

window.multimixerJS = window.multimixerJS || {};
...
multimixerJS.mxrMouseHandlersAttached = false;
...
multimixerJS.initPresetButtons = function () {
	...
	// Stereo Split Button
	stereoSplitBtn.addEventListener("click", () => {
	...
	partPredomBtn.addEventListener("click", () => {
	...
	voiceRadios.forEach((radio) => {
		radio.addEventListener("change", (e) => {
			// c•onsole.log(`radio("click", (e: ${e.target})`);
	...
	// Helper function to update audio nodes
	function updateAudioNodesX(mxrTrack) {
		if (multimixerJS.mxrIsPlaying) {
			if (mxrTrack.dialAngle) {
			}
			if (mxrTrack.panNode) {
...
multimixerJS.pointerDownHandler = function (e) {
	...
	const targetType = multimixerJS.getTargetType(e);
	...
	if (targetType === "timeline") {
		...
		const seek = multimixerJS.calculateSeek(e);
		if (!seek) return;
		...
		// timelineSliderData.element.setPointerCapture(e.pointerId);
	}

	if (targetType === "dial") {
		// multimixerJS.mxrActiveDial = e.target;
		...
		mxrDial.setPointerCapture(e.pointerId);

		mxrDial.addEventListener("contextmenu", (e) => {
			e.preventDefault(); // ✅ This one actually blocks the context menu
		});
	}

	if (targetType === "slider") {
		multimixerJS.mxrActiveVolumeSlider = e.target;
...
multimixerJS.pointerMoveHandler = function (e) {
...
multimixerJS.pointerUpHandler = function (e) {
	...
	if (targetType === "timeline") {
		...
		// Release pointer capture
		// if (timelineSliderData.element.hasPointerCapture?.(e.pointerId)) {
		// timelineSliderData.element.releasePointerCapture(e.pointerId);
		// }
...
multimixerJS.onPointerUpX = function (e) {
	multimixerJS.pointerUpHandler(e);

	// Release pointer capture
	// if (e.pointerId !== undefined) {
	// 	mxrDial.releasePointerCapture(e.pointerId);
	// }
	...
	// Restore all dials
	// document
	// 	.querySelectorAll(".mxr-dial")
	// 	.forEach((d) => d.classList.remove("dial-disabled", "dragging"));
};
...
multimixerJS.initPlaybackBtns = function () {
	...
	multimixerJS.mxrPlayBtn.addEventListener("click", () => {
	...
	multimixerJS.mxrPauseBtn.addEventListener("click", () => {
	...
	multimixerJS.mxrRewindBtn.addEventListener("click", () => {
	...
	multimixerJS.mxrDownloadBtn.addEventListener("click", async () => {
...
multimixerJS.initTimelineSeekAndDrag = function () {
	...
	mxrPlayer.addEventListener("pointerdown", (e) => {
		multimixerJS.pointerDownHandler(e);
	});
	mxrPlayer.addEventListener("pointerup", (e) => {
		multimixerJS.pointerUpHandler(e);
	});
};

multimixerJS.calculateSeek = function (e) {
	const mxrPlayer = document.getElementById("mxr-player");
	...
	if (
		e.clientY < clickBox.top ||
		e.clientY > clickBox.bottom ||
		e.clientX > clickBox.right // ||
		// e.clientX < clickBox.left
	)
		return null;

	const x = e.clientX - clickBox.left;
...
multimixerJS.getTargetType = function (e) {
	// check the type of e.target
	const eTarget = `${e.target.id || "no-id"}.${e.target.className}`;
