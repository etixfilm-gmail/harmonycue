// c•onsole.log("hc-static-js-sampler.js is loaded");
	...
	class AudioPlayer extends HTMLElement {
		...
		animateTimeline() {
			...
			if (
				...
				// 	this.samplerTimelineSlider: ${this.samplerTimelineSlider}
				// 	this.samplerCanvas: ${this.samplerCanvas}
				// 	`);
				return;
			}
			...
			// );
			// const lines = this.querySelectorAll(".sampler-timeline-slider");
			// c•onsole.log("💥 Found lines:", lines.length);
		...
		attachEvents() {
			...
			this.samplerVolumeSlider.parentNode.addEventListener(
				"click",
				(e) => {
					if (e.target === this.samplerVolumeSlider.parentNode) {
						this.toggleMute();
					}
				},
				false
			...
			if (this.samplerBtn) {
				...
				this.samplerBtn.addEventListener("click", () => {
					// c•onsole.log("🟢 sampler-btn was clicked");
				});
			} else {
				// c•onsole.log(`this.samplerBtn: ${this.samplerBtn}`);
			}
			...
			if (this.samplerCanvasContainer) {
				...
				this.samplerCanvasContainer.addEventListener("pointerdown", () => {
		...
		updateAudioTime() {
			...
			// c•onsole.log(
			// 	`audioCtx.getOutputTimestamp().contextTime: ${this.audioCtx.getOutputTimestamp().contextTime}`
			// );
			// c•onsole.log(
			// 	`audioCtx.getOutputTimestamp().performanceTime: ${this.audioCtx.getOutputTimestamp().performanceTime}`
			// );
		...
		onProgressDragStart(e) {
			...
			this.seekFromEvent(e); // jump to clicked position immediately
			this.samplerTimelineSlider.classList.add("on");
		...
		onProgressDragEnd = (e) => {
			...
			// );
			// c•onsole.log(`this.audio.startTime: ${this.audio.startTime})`);
			// c•onsole.log(`onProgressDragEnd(e.clientX: ${e.clientX})`);
			// c•onsole.log(
		...
		seekFromEvent(e) {
			// c•onsole.log(`seekFromEvent(e.clientX: ${e.clientX})`);
			const rect = this.samplerCanvasContainer.getBoundingClientRect();
